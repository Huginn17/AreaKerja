@extends('admin.sidebar.index')
@section('sidebaradmin')
    <div class="p-4 sm:ml-64" x-data="{ openModal: false, detail: {} }" x-cloak>

        <!-- Header -->
        <header class="w-full flex items-center justify-between px-6 py-3 flex-wrap">
            <p class="font-semibold text-2xl">Catatan Transaksi</p>
        </header>

        <!-- Tabel Detail -->
        <div class="p-4">

            <div class="mb-8">
                <h2 class="text-lg font-semibold mb-2">Riwayat Koin</h2>

                <!-- TABEL RESPONSIF -->
                <div class="rounded-2xl overflow-hidden border overflow-x-auto">
                    <table class="w-full text-sm min-w-[900px]">
                        <thead>
                            <tr class="bg-orange-500 text-white">
                                <th class="px-4 py-2 text-left">No</th>
                                <th class="px-4 py-2 text-left">No. Referensi</th>
                                <th class="px-4 py-2 text-left">User</th>
                                <th class="px-4 py-2 text-left">Email</th>
                                <th class="px-4 py-2 text-left">Bank</th>
                                <th class="px-4 py-2 text-left">Rekening</th>
                                <th class="px-4 py-2 text-left">Harga</th>
                                <th class="px-4 py-2 text-left">Jumlah Koin</th>
                                <th class="px-4 py-2 text-left">Status</th>
                                <th class="px-4 py-2 text-center">Lihat Detail</th>
                            </tr>
                        </thead>

                        <tbody>
                            @foreach ($transaksi as $item)
                                <tr class="border-b-[2px] border-gray-300">

                                    <td class="px-4 py-2">{{ $loop->iteration }}</td>

                                    <td class="px-4 py-2 break-words max-w-[150px]">
                                        {{ $item->no_referensi }}
                                    </td>

                                    <td class="px-4 py-2 break-words max-w-[150px]">
                                        {{ $item->user->username ?? '-' }}
                                    </td>

                                    <td class="px-4 py-2 break-words max-w-[200px]">
                                        {{ $item->user->email ?? '-' }}
                                    </td>

                                    <td class="px-4 py-2">{{ $item->bank->nama_bank ?? '-' }}</td>
                                    <td class="px-4 py-2">{{ $item->bank->no_rek ?? '-' }}</td>

                                    <td class="px-4 py-2">
                                        Rp {{ number_format($item->hargaPembayaran->harga ?? 0, 0, ',', '.') }}
                                    </td>

                                    <td class="px-4 py-2">
                                        {{ $item->hargaPembayaran->jumlah_koin ?? 0 }} Koin
                                    </td>

                                    <td
                                        class="px-4 py-2 font-semibold 
                                {{ $item->status == 'diterima' ? 'text-green-600' : 'text-red-500' }}">
                                        {{ ucfirst($item->status) }}
                                    </td>

                                    <td class="px-4 py-2 text-center">
                                        <button class="text-blue-600 hover:underline"
                                            @click="
                                        detail = {
                                            id: '{{ $item->id }}',
                                            user: '{{ $item->user->username ?? '-' }}',
                                            email: '{{ $item->user->email ?? '-' }}',
                                            sumber_dana: '{{ $item->sumberDana ?? '-' }}',
                                            bank: '{{ $item->bank->nama_bank ?? '-' }}',
                                            rekening: '{{ $item->bank->no_rekening ?? '-' }}',
                                            harga: '{{ number_format($item->hargaPembayaran->harga ?? 0, 0, ',', '.') }}',
                                            koin: '{{ $item->hargaPembayaran->jumlah_koin ?? 0 }}',
                                            status: '{{ ucfirst($item->status) }}',
                                            tanggal: '{{ $item->created_at->format('d M Y H:i') }}'
                                        };
                                        openModal = true;
                                    ">
                                            <i class="ph ph-file-arrow-up text-4xl"></i>
                                        </button>
                                    </td>

                                </tr>
                            @endforeach
                        </tbody>
                    </table>
                </div>

                <!-- Modal Detail -->
                <div x-show="openModal"
                    class="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50 p-4" x-transition>

                    <div
                        class="bg-white rounded-2xl shadow-lg w-full max-w-xs sm:max-w-md p-4 sm:p-6 relative 
                        max-h-[90vh] overflow-y-auto">

                        <!-- Judul -->
                        <h2 class="text-xl font-semibold text-center mb-6"
                            x-text="
                        detail.status == 'Diterima' 
                            ? 'Top Up Berhasil' 
                            : (detail.status == 'Ditolak' 
                                ? 'Top Up Ditolak' 
                                : 'Menunggu Verifikasi')
                    ">
                        </h2>

                        <!-- Icon -->
                        <div class="flex justify-center mb-6">

                            <!-- Diterima -->
                            <template x-if="detail.status == 'Diterima'">
                                <div class="w-16 h-16 rounded-full bg-orange-100 flex items-center justify-center">
                                    <svg class="w-8 h-8 text-orange-500" fill="none" stroke="currentColor"
                                        stroke-width="2" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7" />
                                    </svg>
                                </div>
                            </template>

                            <!-- Ditolak -->
                            <template x-if="detail.status == 'Ditolak'">
                                <div class="w-16 h-16 rounded-full bg-red-100 flex items-center justify-center">
                                    <svg class="w-8 h-8 text-red-500" fill="none" stroke="currentColor" stroke-width="2"
                                        viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12" />
                                    </svg>
                                </div>
                            </template>

                            <!-- Pending -->
                            <template x-if="detail.status == 'Pending'">
                                <div class="w-16 h-16 rounded-full bg-gray-100 flex items-center justify-center">
                                    <svg class="w-8 h-8 text-gray-500 animate-spin" fill="none" stroke="currentColor"
                                        stroke-width="2" viewBox="0 0 24 24">
                                        <circle class="opacity-25" cx="12" cy="12" r="10" stroke-width="4">
                                        </circle>
                                        <path class="opacity-75" fill="currentColor"
                                            d="M4 12a8 8 0 018-8v4a4 4 0 00-4 4H4z"></path>
                                    </svg>
                                </div>
                            </template>

                        </div>

                        <!-- Detail -->
                        <div class="text-sm space-y-2">

                            <div class="flex justify-between break-words">
                                <span class="font-semibold">No. Transaksi</span>
                                <span x-text="detail.id"></span>
                            </div>

                            <div class="flex justify-between items-center break-words">
                                <span class="font-semibold">Status</span>
                                <span
                                    :class="detail.status == 'Diterima' ? 'bg-orange-500' :
                                        (detail.status == 'Ditolak' ? 'bg-red-500' : 'bg-gray-500')"
                                    class="text-white text-xs px-3 py-1 rounded-full" x-text="detail.status"></span>
                            </div>

                            <div class="flex justify-between break-words">
                                <span class="font-semibold">Nama Pengirim</span>
                                <span x-text="detail.user"></span>
                            </div>

                            <div class="flex justify-between break-words">
                                <span class="font-semibold">Nama Penerima</span>
                                <span>Area Kerja</span>
                            </div>

                            <div class="flex justify-between break-words">
                                <span class="font-semibold">Metode Pembayaran</span>
                                <span x-text="detail.sumber_dana"></span>
                            </div>

                            <div class="flex justify-between break-words">
                                <span class="font-semibold">Tgl/Waktu</span>
                                <span x-text="detail.tanggal"></span>
                            </div>

                            <div class="flex justify-between">
                                <span class="font-semibold">Nominal</span>
                                <span>Rp <span x-text="detail.harga"></span></span>
                            </div>

                            <div class="flex justify-between">
                                <span class="font-semibold">Biaya Admin</span>
                                <span>Rp 2.500</span>
                            </div>

                            <div class="border-t border-dashed my-2"></div>

                            <div class="flex justify-between font-semibold">
                                <span>Total Pembayaran</span>
                                <span>Rp <span x-text="parseInt(detail.harga.replaceAll('.', '')) + 2500"></span></span>
                            </div>
                        </div>

                        <!-- Logo -->
                        <div class="flex justify-center mt-8">
                            <img src="{{ asset('images/logoarea.png') }}" alt="Logo" class="w-16">
                        </div>

                        <div class="mt-6 flex justify-end">
                            <button @click="openModal = false" class="bg-gray-200 px-4 py-2 rounded-lg hover:bg-gray-300">
                                Tutup
                            </button>
                        </div>

                    </div>
                </div>
            </div>

        </div>

    </div>

    <!-- Alpine.js -->
    <script src="//unpkg.com/alpinejs" defer></script>
@endsection
