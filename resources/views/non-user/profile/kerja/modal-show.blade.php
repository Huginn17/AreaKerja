...
<div id="show-kerja" tabindex="-1" aria-hidden="true"
    class="hidden fixed inset-0 bg-black/50 backdrop-blur-sm overflow-y-auto flex justify-center items-center z-50 p-3">

    <div class="relative w-full max-w-sm md:max-w-md p-3 md:p-4">
        <!-- Modal content -->
            ...
            <div class="p-3 md:p-5">
                @if (Auth::user()->role === 'pelamar')
                    @foreach (Auth::user()->pelamar->pengalaman_kerja as $kerja)
                        <div class="mb-6 border-b pb-4 last:border-none last:pb-0">

                            <div class="flex justify-between items-start">
                                <h3 class="font-semibold text-gray-800 text-sm md:text-md ml-2 md:ml-5">
                                    {{ $kerja->posisi_pekerjaan }} - {{ $kerja->nama_perusahaan }} <br>
                                    ({{ $kerja->tahun_awal }} - {{ $kerja->tahun_akhir }})
                                </h3>

                                <a href="{{ route('kerja.edit', $kerja->id) }}" class="mr-2 md:mr-6 mt-1 block">
                            ...
                            <div class="flex gap-2 mt-3 ml-2 md:ml-5">
                                <form action="{{ route('kerja.destroy', $kerja->id) }}" method="POST"
                                    onsubmit="return confirm('Yakin Pengalaman Kerja ini?')">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit"
                                        class="px-3 py-1 bg-red-500 hover:bg-red-600 text-white text-xs md:text-sm rounded-lg">
                                        Hapus
                                    </button>
                                </form>
                            </div>
