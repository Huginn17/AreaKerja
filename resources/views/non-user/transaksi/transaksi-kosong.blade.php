    ...
    <table class="w-full bg-white border-collapse">
        ...
        <tbody>
            @forelse ($transaksi as $t)
                <tr class="border-b">
                    <td class="p-3 text-center">{{ $t->no_referensi }}</td>
                    <td class="p-3 text-center">{{ $t->bank->nama_bank ?? '-' }}</td>
                    <td class="p-3 text-center">{{ $t->pesanan ?? '-' }}</td>
                    <td class="p-3 text-center">Rp {{ number_format($t->total, 0, ',', '.') }}</td>
                    <td
                    ...
                    <td class="p-3 text-center">{{ $t->created_at->format('d M Y H:i') }}</td>
                </tr>
            @empty
                <tr>
                    <div class="border-gray-200 py-32 max-w-2xl mx-auto border rounded-3xl mt-8 mb-8 shadow">
                        <p class="text-xl text-gray-400 text-center font-medium">Anda belum memiliki <br>transaksi apapun
                        </p>
                        <div class="flex items-center justify-center mt-4">
                            <svg width="90" height="78" viewBox="0 0 100 98" fill="none"
