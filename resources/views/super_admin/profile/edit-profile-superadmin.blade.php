        ...
        <div class="max-w-5xl mx-auto bg-white border border-gray-400 rounded-lg p-6 shadow-sm">
            ...
            <form action="{{ route('superadmin.update.profile', Auth::user()->superadmin->id) }}"
                ...
                <div class="flex items-center gap-4 mb-6">
                    @if (Auth::user()->superadmin->img_profile)
                        <img id="pa" class="w-24 h-24 object-cover rounded-full"
                            src="{{ asset('storage/' . Auth::user()->superadmin->img_profile) }}" alt="Profile">
                    @else
                        <img id="pa" class="w-24 h-24 object-cover rounded-full"
                            src="https://ui-avatars.com/api/?name={{ urlencode(Auth::user()->username) }}&background=random&color=fff&size=128"
                    ...
                    <div class="flex flex-col gap-3 scale-90">
                        ...
                        <button type="button" onclick="document.getElementById('fileinputsuperadmin').click();"
                            class="flex items-center gap-2 px-4 py-2 text-sm border-2 border-orange-500 bg-orange-500 text-white rounded-md">
                            <!-- Icon upload -->
                        ...
                        <button type="button"
                            ...
                            class="flex items-center gap-2 px-4 py-2 mt-1 text-sm border-2 border-orange-500 text-orange-600 rounded-md hover:bg-gray-100">
                            <!-- Icon trash -->
                ...
                <div>
                    ...
                    <input type="text" value="{{ Auth::user()->email }}" name="email"
                        class="w-full border text-black border-gray-300 shadow rounded-md px-3 py-2 focus:ring-2 focus:ring-orange-500 focus:outline-none">
                </div>
                    ...
                    <div>
                        ...
                        <select name="provinsi"
                            ...
                            @if (Auth::user()->superadmin->provinsi)
                                <option selected value="{{ Auth::user()->superadmin->provinsi }}">
                                    {{ Auth::user()->superadmin->provinsi }}</option>
                            @else
                    ...
                    <div>
                        ...
                        <select name="kota"
                            ...
                            @if (Auth::user()->superadmin->kota)
                                <option selected value="{{ Auth::user()->superadmin->kota }}">
                                    {{ Auth::user()->superadmin->kota }}</option>
                            @else
                                <option selected disabled>Pilihan Kota</option>
                                <div>
                    ...
                    <div>
                        ...
                        <select name="kecamatan"
                            ...
                            @if (Auth::user()->superadmin->kecamatan)
                                <option selected value="{{ Auth::user()->superadmin->kecamatan }}">
                                    {{ Auth::user()->superadmin->kecamatan }}</option>
                            @else
                                <option selected disabled>Pilihan Kecamatan</option>
                                <option value="Maguwoharjo">Maguwoharjo</option>
                            @endif
                        </select>
                    </div>
                </div>
